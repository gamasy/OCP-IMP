#!/bin/sh
#
# Portworx air-gapped images helper v25.6.0
# - script copies Portworx container images to air-gapped hosts that do not have direct access to Internet
# - generated from https://install.portworx.com/25.6.0/air-gapped?kbver=1.31.9 on 2025-07-18T08:16:53Z
#
...
IMAGES=""
IMAGES="$IMAGES docker.io/portworx/px-csi:25.6.0"
IMAGES="$IMAGES docker.io/portworx/oci-monitor:25.6.0"
IMAGES="$IMAGES docker.io/purestorage/ccm-go:1.2.2"
IMAGES="$IMAGES docker.io/purestorage/realtime-metrics:1.0.29"
IMAGES="$IMAGES docker.io/purestorage/telemetry-envoy:1.1.16"
IMAGES="$IMAGES docker.io/portworx/log-upload:px-1.1.40"
IMAGES="$IMAGES docker.io/portworx/px-operator:25.2.2"
IMAGES="$IMAGES registry.k8s.io/pause:3.1"
IMAGES="$IMAGES docker.io/portworx/portworx-dynamic-plugin:1.1.3"
IMAGES="$IMAGES docker.io/nginxinc/nginx-unprivileged:1.27-alpine3.20-slim"
IMAGES="$IMAGES registry.k8s.io/sig-storage/csi-node-driver-registrar:v2.12.0"
IMAGES="$IMAGES registry.k8s.io/sig-storage/csi-provisioner:v5.1.0"
IMAGES="$IMAGES registry.k8s.io/sig-storage/csi-resizer:v1.12.0"
IMAGES="$IMAGES registry.k8s.io/sig-storage/csi-snapshotter:v8.1.0"
IMAGES="$IMAGES registry.k8s.io/sig-storage/snapshot-controller:v8.1.0"
IMAGES="$IMAGES quay.io/prometheus/prometheus:v3.4.0"
IMAGES="$IMAGES quay.io/prometheus-operator/prometheus-operator:v0.82.2"
IMAGES="$IMAGES quay.io/prometheus-operator/prometheus-config-reloader:v0.82.2"
IMAGES="$IMAGES quay.io/prometheus/alertmanager:v0.28.1"
...
push_images_registry() {
    ...
    if echo $reg | grep -q /; then
        ...
        trans() {
            i2="${1##docker.io/}" #remove docker.io
            i3="${i2##registry.k8s.io/}" #remove registry.k8s.io from i2
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        -v)
            ...
            ;;
        -V|--version)
            echo "Portworx air-gapped images helper v25.6.0"
            shift $#
            break ;;
        -h|--help)
            usage
